...
1.  Under Windows run kicad.exe. Under Linux type 'kicad' in your
    ...
    to give you an idea of how the main tools are used.
+
...
3.  Let's begin by creating a schematic. Start the schematic editor
    __Eeschema__, image:icons/eeschema.png[Eeschema]. It is the first
    button from the left.

4.  Click on the 'Page Settings' icon
    image:icons/sheetset.png[Sheet settings icon] on the top
...
5.  We will now place our first component. Click on the 'Place
    component' icon image:icons/add_component.png[Add component Icon]
...
+
image:images/eeschema_placed_components.png[eeschema_placed_components]
...
+
image:images/eeschema_myconn3.png[eeschema_myconn3]

26. It is time to place the power and ground symbols. Click on the
    'Place a power port' button image:icons/add_power.png[add_power_png] on
...
+
image:images/eeschema_placed_components2.png[eeschema_placed_components2]

29. Next, we will wire all our components. Click on the 'Place wire'
    icon image:icons/add_line.png[Place wire] on the right
    toolbar.
+
...
+
image:images/eeschema_first_wire.png[eeschema_first_wire]
...
31. Repeat this process and wire up all the other components as shown
    ...
    below.
+
image:images/eeschema_all_wires.png[eeschema_all_wires]
...
32. We will now consider an alternative way of making a connection
    ...
    net name' icon image:icons/label.png[label_png] on the right
...
+
image:images/eeschema_all_labels.png[eeschema_all_labels]
...
41. It is now necessary to add a 'Power Flag' to indicate to KiCad
    ...
    VCC as shown below.
+
image:images/eeschema_powerflag.png[eeschema_powerflag]
+
...
42. Sometimes it is good to write comments here and there. To add
    ...
    icon image:icons/add_text.png[add_text_png] on the right toolbar.
...
43. All components now need to have unique identifiers. In fact, many
    ...
    schematic' icon image:icons/annotate.png[annotate_png].
...
45. We will now check our schematic for errors. Click on the 'Perform
    Electric Rules Check' icon image:icons/erc.png[erc_png]. Click on
...
46. The schematic is now finished. We can now create a Netlist file to
    ...
    'Netlist generation' icon image:icons/netlist.png[netlist_png] on
...
47. You can now quit the schematic editor. From KiCad, click on the
    'Run Cvpcb' icon
    image:icons/icon_cvpcb_small.png[icon_cvpcb_small_png] on the top
...
48. _Cvpcb_ allows you to link all the components in your schematic
    ...
    it. image:icons/cvpcb.png[cvpcb_png]
...
49. It is possible that the pane on the right shows only a selected
    ...
    icon image:icons/module_full_list.png[module_full_list_png] to
...
51. If you are interested in knowing what the footprint you are
    ...
    image:icons/show_footprint.png[show_footprint_png] for a preview
    of the current footprint. Alternatively, click on the 'Display
    footprint list documentation' icon
    image:icons/datasheet.png[datasheet_png] and you will get a
...
52. You are done. You can now update your netlist file with all the
    ...
    icon image:icons/save.png[Save icon].  Your netlist file has now
...
56. To create a Bill Of Materials (BOM), go to the _EESchema_ schematic 
    ...
    image:icons/bom.png[bom_png] on the top toolbar.
...
3.  Repeat the same labelling action on the two other connectors
    ...
    image:icons/add_line2bus.png[Place wire to bus entry] and bus
    line using the icon image:icons/add_bus2bus.png[Place bus to bus
...
6.  Connect and label CONN_4 using the labelling method explained
    ...
    image:icons/add_line2bus.png[add_line2bus_png] and to a bus line
    using the icon image:icons/add_bus.png[add_bus_png]. See Figure
    4.
...
9.  What we can now do is connect bus a[1..4] with bus b[1..4] using a
    bus line with the button image:icons/add_bus.png[add_bus_png].
...
11. The 'Repeat last item' option accessible via the Ins key has also
    ...
    using the icon image:icons/add_line2bus.png[add_line2bus_png].
+
image:images/eeschema_repeat_bus.png[eeschema_repeat_bus]
...
1.  From the KiCad project manager, click on the 'PCBNew' icon
    image:icons/pcbnew.png[pcbnew_png]. The 'PCBNew' window will
...
2.  Begin by entering some schematic information. Click on the 'Page
    settings' icon image:icons/sheetset.png[sheetset_png] on the top
...
5.  Now we will import the netlist file. Click on the 'Read Netlist'
    icon image:icons/netlist.png[netlist_png] on the top
...
8.  All components are connected via a thin group of wires called
    ...
    image:icons/general_ratsnest.png[general_ratsnest_png] is
...
+
image:images/pcbnew_placement.png[pcbnew_placement]
...
11. Now we will define the edge of the PCB. Select 'Edge.Cuts' from
    ...
    image:icons/add_dashed_line.png[add_dashed_line_png] on the right
...
15. Click on the 'Add Tracks and vias' icon
    image:icons/add_tracks.png[add_tracks_png] on the right
...
+
image:images/pcbnew_track_width.png[pcbnew_track_width]
...
+
image:images/pcbnew_most_tracks.png[pcbnew_most_tracks]
...
21. Let's now run a track on the other copper side of the PCB. Select
    ...
    image:icons/add_tracks.png[add_tracks_png]. Draw a track between
...
22. **Go from pin A to pin B by changing layer**. It is possible to
    ...
    track.
+
image:images/pcbnew_via.png[pcbnew_via]

23. When you want to inspect a particular connection you can click on
    the 'Net highlight' icon
    image:icons/net_highlight.png[net_highlight_png] on the right
...
24. Now we will make a ground plane that will be connected to all GND
    ...
    image:icons/add_zone.png[add_zone_png] on the right toolbar. We
...
+
image:images/pcbnew_filled.png[pcbnew_filled]

26. Run the design rules checker by clicking on the 'Perform Design
    Rules Check' icon image:icons/erc.png[erc_png] on the top
...
1.  From KiCad, open the _PCBNew_ software tool and load your board
    ...
    image:icons/open_document.png[open_document_png].
...
1.  Proceed by clicking on the 'Plot' button. To view all your Gerber
    ...
    image:icons/gerber_file.png[gerber_file_png]. Load all generated Gerber
...
'Add tracks' icon image:icons/add_tracks.png[Add Track icon] on the
right toolbar.
...
2.  Now we can start creating our new component. From KiCad, start
    ...
    image:icons/libedit.png[libedit_png] and then click on the 'New
    component' icon
    image:icons/new_component.png[new_component_png]. The Component
...
7.  Next, draw the contour of the component. Click on the 'Add
    rectangle' icon
    image:icons/add_rectangle.png[add_rectangle_png]. We want to draw
    ...
    be.
+
image:images/component_outline.png[component_outline]

8.  Save the component in your library __myLib.lib__. Click on the
    'New Library' icon image:icons/new_library.png[new_library_png],
...
10. Click on the 'Select working library' icon
    image:icons/library.png[library_png]. In the Select Library
...
11. Click on the 'Update current component in current library' icon
    image:icons/save_part_in_mem.png[save_part_in_mem_png] in the top
    toolbar. Save all changes by clicking on the 'Save current loaded
    library on disk' icon
    image:icons/save_library.png[save_library_png] in the top
...
1.  From KiCad, start __EESchema__, click on the 'Library Editor' icon
    image:icons/libedit.png[libedit_png], click on the 'Select
    working library' icon image:icons/library.png[library_png] and
    choose the library 'device'. Click on 'Load component to edit from
    the current lib' icon
    image:icons/import_cmp_from_lib.png[import_cmp_from_lib_png] and
    import the 'RELAY_2RT'.

2.  Click on the 'Export component' icon
    image:icons/export.png[export_png], navigate into the _library/_
...
5.  Have a look at it using KiCad. From the KiCad project manager, start
    __EESchema__, click on the 'Library Editor' icon
    image:icons/libedit.png[libedit_png], click on the 'Import Component'
    icon image:icons/import.png[import_png], navigate to _demo1/library/_
    and select _myLib.lib._
+
image:images/library_myconn3.png[library_myconn3]
...
image:images/high_pin_count.png[high_pin_count]
...
1.  From the KiCad project manager start the _PCBnew_ tool. Click on
    ...
    image:icons/edit_module.png[edit_module_png] on the top
...
2.  We are going to save the new footprint in the footprint library
    ...
    image:icons/library.png[library_png] on the top toolbar. Select
    the 'connect' library, though you can choose a different location
    if you want.

[[myconn3]]
3.  Click on the 'New Module' icon
    image:icons/new_footprint.png[new_footprint_png] on the top
...
4.  Select the 'Add Pads' icon image:icons/pad.png[pad_png] on the
...
10. Now add a footprint contour. Click on the 'Add graphic line or
    polygon' button image:icons/add_polygon.png[add_polygon_png] in
    the right toolbar. Draw an outline of the connector around the
    component.

11. Click on the 'Save Module in working directory' icon
    image:icons/save_library.png[save_library_png] on the top
...
3.  From KiCad, start __EESchema__, make your modifications by
    ...
    the icon image:icons/save.png[Save icon] and c lick on the
    'Netlist generation' icon image:icons/netlist.png[netlist_png] on
    the top toolbar.
...
8.  The old, already routed, board should automatically open. Let's
    ...
    image:icons/netlist.png[netlist_png] on the top toolbar.
